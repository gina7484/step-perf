...
/// emitted on *every* input rather than only at the end of a group. The first
/// output is exclusive (the state before the current input), and the second is
/// inclusive (the state after the current input). Two
...
/// - **Both results are stored.** `write_back_mu` charges store cycles for both
///   output tiles on each input, not just at the group boundary.
///
...
pub struct Scan<E, T: DAMType, OT: DAMType> {
    ...
    exclusive_out_stream: Sender<Elem<Tile<OT>>>,
    inclusive_out_stream: Sender<Elem<Tile<OT>>>,
    fn1: Fold<T, OT>,
    ...
    Elem<Tile<OT>>: DAMType,
{
    pub fn new(
        ...
        let ctx = Self {
            in1_stream,
            in2_stream,
            exclusive_out_stream,
            inclusive_out_stream,
            fn1,
            fn2,
            init_accum,
            rank,
        ...
        ctx.exclusive_out_stream.attach_sender(&ctx);
        ctx.inclusive_out_stream.attach_sender(&ctx);
        ctx
    }

    /// Fold one input and return its exclusive and inclusive accumulator values.
    ///
    /// `group_end` resets the accumulator afterwards. The fold still runs on
    /// that last input even though the reset discards the new state.
    fn process(
        &mut self,
        data1: Tile<T>,
        data2: Option<Tile<T>>,
        ...
        let exclusive_out = accumulator.clone();
        ...
        let inclusive_out = folded.clone();
        *accumulator = if group_end {
        ...
        let store_cycles = if self.config.write_back_mu {
            div_ceil(exclusive_out.size_in_bytes() as u64, PMU_BW)
                + div_ceil(inclusive_out.size_in_bytes() as u64, PMU_BW)
        } else {
            0
        };
    ...
    Elem<Tile<OT>>: DAMType,
{
    fn run(&mut self) {
        ...
        loop {
            ...
            let (exclusive_out, inclusive_out) =
                self.process(data1, data2, &mut accumulator, group_end);

            let exclusive_data = match level {
                None => Elem::Val(exclusive_out),
                Some(level) => Elem::ValStop(exclusive_out, level),
            };
            let inclusive_data = match level {
                None => Elem::Val(inclusive_out),
                Some(level) => Elem::ValStop(inclusive_out, level),
            };
            let output_time = self.time.tick();
            self.exclusive_out_stream
                .enqueue(
                    &self.time,
                    ChannelElement {
                        time: output_time,
...
mod tests {
    ...
    /// Drive `in1` (and `in2`, when given) through one `Scan` and return its
    /// exclusive and inclusive streams. `add` is used for both folds.
    fn run_scan(
        in1: Vec<Elem<Tile<i32>>>,
        in2: Option<Vec<Elem<Tile<i32>>>>,
        rank: StopType,
    ) -> (Vec<Elem<Tile<i32>>>, Vec<Elem<Tile<i32>>>) {
        ...
        let (exclusive_out_snd, exclusive_out_rcv) = ctx.unbounded();
        let (inclusive_out_snd, inclusive_out_rcv) = ctx.unbounded();
        ...
        let exclusive_collected = Arc::new(Mutex::new(Vec::new()));
        let inclusive_collected = Arc::new(Mutex::new(Vec::new()));
        ...
        ctx.add_child(Collect::new(exclusive_out_rcv, exclusive_collected.clone()));
        ctx.add_child(Collect::new(inclusive_out_rcv, inclusive_collected.clone()));
        ctx.initialize(Default::default())
            .unwrap()
            .run(Default::default());

        let exclusive = exclusive_collected.lock().unwrap().clone();
        let inclusive = inclusive_collected.lock().unwrap().clone();
        (exclusive, inclusive)
    }

    // One group of three emits the state before and after every fold.
    #[test]
    fn test_scan_emits_exclusive_and_inclusive_running_sums() {
        let (exclusive, inclusive) =
            run_scan(stream(&[(1, None), (2, None), (3, Some(1))]), None, 1);
        assert_eq!(exclusive, stream(&[(0, None), (1, None), (3, Some(1))]));
        assert_eq!(inclusive, stream(&[(1, None), (3, None), (6, Some(1))]));
    }
    ...
    fn test_scan_restarts_at_each_group() {
        ...
        let (exclusive, inclusive) = run_scan(input, None, 1);
        assert_eq!(
            exclusive,
            stream(&[
                (0, None),
    ...
    fn test_rank2_scan_carries_across_the_inner_stop() {
        ...
        let (exclusive, inclusive) = run_scan(input, None, 2);
        assert_eq!(
            exclusive,
            stream(&[
                (0, None),
    ...
    fn test_two_inputs_chain_both_folds() {
        let (exclusive, inclusive) = run_scan(
            stream(&[(1, None), (2, None), (3, Some(1))]),
            Some(stream(&[(10, None), (20, None), (30, Some(1))])),
            1,
        );
        assert_eq!(exclusive, stream(&[(0, None), (11, None), (33, Some(1))]));
        assert_eq!(inclusive, stream(&[(11, None), (33, None), (66, Some(1))]));
    }
}
